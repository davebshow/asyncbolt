...
`asyncbolt` is an implementation of the [Bolt](https://boltprotocol.org/) client/server protocol for Python's [Asyncio](https://docs.python.org/3/library/asyncio.html)
module. While the `asyncbolt.session.ClientSession` object aims to be fully compatible with Neo4j (or any other server that
speaks Bolt), it does not (will not) attempt to provide a fully featured asynchronous Neo4j client.
...
Use the `asyncbolt.ClientSession` object to talk to the server:
```python
import asyncbolt

client = await asyncbolt.connect('tcp://localhost:7687')
async for msg in client.run("RETURN 1 AS num", {}):
    print(msg)
# ClientResponse(fields=[1], metadata={'result_available_after': 63, 'fields': ['num']}, eof=False)
```
...
'b'\x00\x1d\xb2\x01\x8dAsyncBolt/1.0\xa1\x86scheme\x84none\x00\x00''
...
Data received:
b'\x00(\xb1p\xa2\xd0\x16result_available_after?\x86fields\x91\x83num\x00\x00\x00\x04\xb1q\x91\x01\x00\x00\x00"\xb1p\xa2\xd0\x15result_consumed_after\x03\x84type\x81r\x00\x00'
...
Data received:
b'\x00$\xb1p\xa2\xd0\x16result_available_after:\x86fields\x90\x00\x00\x008\xb1p\xa3\x85stats\xa1\x8dnodes-created\x01\xd0\x15result_consumed_after\x00\x84type\x81w\x00\x00'
...
Data received:
b'\x00&\xb1p\xa2\xd0\x16result_available_after\x1b\x86fields\x91\x81n\x00\x00\x00\x08\xb1q\x91\xb3N\x00\x90\xa0\x00\x00\x00"\xb1p\xa2\xd0\x15result_consumed_after\x13\x84type\x81r\x00\x00'

Client received message: 

ClientResponse(fields=[Node(signature=78, nodeIdentity=0, labels=[], properties={})], metadata={'result_available_after': 27, 'fields': ['n']}, eof=False)

Finished in 0.03721186199982185
